class GameScene extends Phaser.Scene{
    ...
    create(){
        ...
        const rect =new Phaser.Geom.Rectangle(0,-300,game.scale.baseSize.width,
            game.scale.baseSize.height +300);
        ...
        this.cleatBlockContainer = this.add.container(100,-1900);
        for(var i=0;i<9;i++){
            var image =this.add.image(i*this.blockWidth,0,'tile');
            this.cleatBlockContainer.add(image);
        }
    }
        ...
        switch (event.keyCode) {
            case Phaser.Input.Keyboard.KeyCodes.LEFT:
                this.checkBlock(this.blockContainer.x-this.blockWidth,this.blockContainer.y,
                    function(checksum){
                        if(checksum==0)this.blockContainer.setPosition(this.blockContainer.x-this.blockWidth,this.blockContainer.y)
                    }.bind(this)
                )
            break;
            ...
            case Phaser.Input.Keyboard.KeyCodes.DOWN:
                this.checkBlock(this.blockContainer.x,this.blockContainer.y +this.blockHeight,
                    function(checksum){
                        if(checksum==0)this.blockContainer.setPosition(this.blockContainer.x,this.blockContainer.y+this.blockHeight)
                    }.bind(this)
                )
            break;
    ...
    onTimerEvent(){
        this.checkBlock(this.blockContainer.x,this.blockContainer.y+this.blockHeight,
            function(checksum){
                if(checksum ==0){
                    this.blockContainer.setPosition(this.blockContainer.x,
                        this.blockContainer.y + this.blockHeight);
                }else{
                    this.addBlock();
    ...
    newBlock(x,y,type){
        ...
        this.blockType =type;
        this.blockContainer.x=x;
        this.blockContainer.y=y;
        this.blockContainer.setPosition(x,y);
        ...
        this.checkBlock(x,y,function(checksum){
            if(checksum>0){
                if(y<0){
                    alert("Game Over!!!");
                    this.resetGame();
    ...
    addBlock(){
        this.blockContainer.each(function(obj){
            var image=this.physics.add.image(this.blockContainer.x+obj.x,
                this.blockContainer.y+obj.y,'tile');
            image.setTint(this.color);
    ...
    checkRotateBlock(callback){
        ...
        new TweensCheck(this,this.blockContainer.x,this.blockContainer.y,
            this.blockWidth/4,this.blockHeight/4,container,function(checksum){
                callback(checksum);
            }.bind(this)
        );
    ...
    retateBlock(){
        this.checkRotateBlock(function(checksum){
            if(checksum ==0){
    ...
    checkClearBlock(x,y){
        new TweensCheck(this,x,y,
            this.blockWidth/4,this.blockHeight/4,
            this.cleatBlockContainer,
            function(checksum){
                if(checksum>8){
                    var overlap=this.physics.overlapRect(
                        x,y,this.blockWidth*8,this.blockHeight/4);
                    overlap.forEach(function(obj){
                        obj.gameObject.destroy();
                    }.bind(this));
                }else if(checksum==0){
                    var overlap=this.physics.overlapRect(
                        x,y-this.blockHeight,this.blockWidth*8,this.blockHeight/4);
                    overlap.forEach(function(obj){
